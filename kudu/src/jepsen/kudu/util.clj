...
(defn path
  [& components]
  (str/join "/" (reduce (fn [col e] (conj col e)) [] components)))

;; TODO make it possible to set the version via some option
...
;; The set of the required packages -- the rest is installed as dependencies.
(def kudu-required-packages [:kudu-master :kudu-tserver :ntp])

; TODO: uncomment for Kudu builds
...
(defn kudu-master-see-tservers?
  "Whether master sees the specified number of tablet servers."
  [test node tservers-count]
  (def pattern (str "Fetched info from all "
...
(defn kudu-tserver-in-service?
  "Is the Kudu tserver process at the specified node in service already?
  This function should be called in the context of already established SSH
  session at the node."
  [test node]
...
(defn start-kudu-master
  ...
  [test node]
  (def use-svc-scripts? (:use-packages? test))
  ...
  (if use-svc-scripts?
    ...
    (c/exec :sudo :-u kudu-uname :start-stop-daemon
            ...
            :>> (path kudu-log-dir "kudu-master.out") (c/lit "2>&1")))

  ;; Wait for master services avaiable (i.e. awaiting for the catalog manager).
  (loop [iteration 0]
  ...
  ;; Wait till the master sees all tservers in the cluster. Otherwise the client
  ;; would not be able to create table with desired replication factor if the
  ;; not all tservers sent their registration at this point.
  (def tservers-count (count (:tservers test)))
  (loop [iteration 0]
...
(defn stop-kudu-master
  ...
  (meh (c/exec :rm :-rf (path kudu-home-dir "master"))))
...
(defn start-kudu-tserver
  ...
  (if use-svc-scripts?
    ...
    (c/exec :sudo :-u kudu-uname :start-stop-daemon
            ...
            :>> (path kudu-log-dir "kudu-tserver.out") (c/lit "2>&1")))
...
(defn stop-kudu-tserver
  ...
  (meh (c/exec :rm :-rf (path kudu-home-dir "tserver"))))


(defn kudu-cfg-master
  [test]
  (def data-path (path kudu-home-dir "master"))
  (def flags [(str "--fs_wal_dir=" data-path)
  ...
  (when (> (count (:masters test)) 1)
    (conj flags (str "--master_addresses="
                     (concatenate-addresses master-rpc-port (:masters test)))))
...
(defn kudu-cfg-tserver
  [test]
  (def data-path (path kudu-home-dir "tserver"))
  (def flags [(str "--fs_wal_dir=" data-path)
              (str "--fs_data_dirs=" data-path)
              (str "--log_dir=" kudu-log-dir)
...
(defn ntp-server-config
  []
  (str/join "\n" (concat ntp-common-opts ntp-server-opts)))


(defn ntp-slave-config
  [masters]
  (str/join "\n" (concat ntp-common-opts
                         [(str "server " (name (first masters))
                               " burst iburst prefer minpoll 4 maxpoll 4")])))

(defn prepare-node-with-pkgs
  "Prepare Kudu node: install Kudu using packages."
  [test node]
...
(defn prepare-node-with-binaries
  ...
  (when (.contains (:masters test) node)
    (c/exec :mkdir :-p (path kudu-home-dir "master")))
  (when (.contains (:tservers test) node)
    (c/exec :mkdir :-p (path kudu-home-dir "tserver")))
  ...
  (when (.contains (:masters test) node)
    (c/upload (path kudu-build-dir "bin" "kudu-master") kudu-target-sbin-dir)
    (c/exec :chmod 755 (path kudu-target-sbin-dir "kudu-master")))
  (when (.contains (:tservers test) node)
    (c/upload (path kudu-build-dir "bin" "kudu-tserver") kudu-target-sbin-dir)
    (c/exec :chmod 755 (path kudu-target-sbin-dir "kudu-tserver")))
  (c/upload (path kudu-build-dir "bin" "kudu") kudu-target-bin-dir)
  (c/exec :chmod 755 (path kudu-target-bin-dir "kudu")))
